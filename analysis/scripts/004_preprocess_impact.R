...
impact <- fread(
  "/Users/artzo/Documents/8_asset_files/IMPACT/mutations/20250430_data_mutations_extended.txt.gz"
)
...
samples <- patient_meta_mod %>%
  ...
  pivot_longer(
    cols = c(impact_baseline, impact_progression),
...
# export smaller version to upload with publication
impact_mod_upload <- impact_mod |>
  select(
    patient_id,
    hugo_symbol,
    hgv_sp_short
...
write.table(
  ...
  file = paste0(
    ...
    "/",
    project_folder,
    "/data/processed/004_impact_mod_upload.txt"
  ),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
