...
source(paste0(project_root, "/", 
              project_folder, 
              "/analysis/scripts/utils/001_preprocess_utils.R"))

# Guardant OMNI mutations
guardant_mod <- fread(paste0(project_root, "/",
                             project_folder,
                             "/data/processed/002_guardant_mod.txt"))

# IMPACT mutations
impact_mod <- fread(paste0(project_root, "/",
                           project_folder,
                           "/data/processed/004_impact_mod.txt"))

# panel gene list
panel_gene_list <- fread(paste0(project_root, "/",
                                project_folder,
...
# run the function
annotated_mutations <- annotate_patient_mutations(guardant_mod, 
                                                  impact_mod, 
                                                  panel_gene_list, 
                                                  unique_patient_ids)
...
annotated_mutations_mod <- annotated_mutations_mod %>% 
  mutate(reference_allele = str_split_fixed(mut_nt, ">", 2)[,1], 
...
write.table(annotated_mutations_progression, 
            file = paste0(
              project_root, "/", project_folder, "/data/processed/005_annotated_mutations_progression.txt"), 
            sep = "\t", 
            quote = FALSE, 
            row.names = FALSE)

# this dataset contains all mutations from all sample types (i.e. it has the lost mutations from IMPACT)
write.table(annotated_mutations_all, 
            file = paste0(
              project_root, "/", project_folder, "/data/processed/005_annotated_mutations_all.txt"), 
            sep = "\t", 
            quote = FALSE, 
            row.names = FALSE)
